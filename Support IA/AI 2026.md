<img src="Photos/ENCG 2.jpeg" style="height:200px;margin-right:200px; float:left; border-radius:10px;"/>

---
# HOUTATE SAÏD --- **Numéro d'étudiant** : 24010355 
<img src="Photos/WhatsApp Image 2025-12-02 at 19.28.38.jpeg" style="height:300px;margin-right:300px; float:left; border-radius:10px;"/>

---
# JAMAL YASSINE --- **Numéro d'étudiant** : 22007655 
<img src="Photos/WhatsApp Image 2026-02-17 at 15.45.58.jpeg" style="height:300px;margin-right:300px; float:left; border-radius:10px;"/>

---

**Classe** : CAC2

**Date :** : 17 Février 2026

<br clear="left"/>

---

# Compte rendu
## TP1 - Apprentissage Supervisé :

---
---
# PARTIE 1
---
---

## Statistiques et Loi Normale en Finance
### Analyse Risque Portefeuille et Calcul VaR

---

## 📋 Contexte

Analyse de deux portefeuilles actions pour un client disposant de **€500,000** avec une tolérance de perte maximale de **€50,000** (10% du capital) sur un horizon annuel avec 95% de confiance.

- **Portefeuille A (CONSERVATIVE)** : Actions blue-chip européennes (CAC 40, DAX)
- **Portefeuille B (AGRESSIF)** : Actions small-cap tech émergentes

---

## Question 1.1 - Statistiques Descriptives

### Code Python

```python
import numpy as np
import pandas as pd
from scipy import stats

# Données historiques (rendements mensuels %, 24 mois)
rendements_A = np.array([
    1.2, 0.8, -0.5, 1.5, 0.9, 1.1, 0.7, 1.3, 1.0, 0.6, 1.4, 0.8,
    1.1, 0.9, -0.3, 1.2, 1.0, 1.5, 0.8, 1.3, 0.9, 1.1, 1.2, 1.0
])

rendements_B = np.array([
    4.5, -2.1, 6.2, -3.5, 5.8, 7.1, -1.8, 4.9, 3.2, -4.2, 8.5, -2.7,
    5.1, 6.8, -3.1, 7.3, 4.5, -2.9, 6.7, 5.3, -3.8, 7.9, 4.2, 5.5
])

def calculer_stats_portefeuille(rendements, nom):
    # a) Moyenne mensuelle
    moyenne_mensuelle = np.mean(rendements)

    # b) Écart-type mensuel (ddof=1 pour échantillon)
    ecart_type_mensuel = np.std(rendements, ddof=1)

    # c) Médiane
    mediane = np.median(rendements)

    # d) Rendement annualisé (capitalisation composée)
    # Formule : (1 + r_mensuel/100)^12 - 1
    rendement_annuel = ((1 + moyenne_mensuelle/100)**12 - 1) * 100

    # e) Volatilité annualisée
    # Formule : σ_annuel = σ_mensuel × √12
    volatilite_annuelle = ecart_type_mensuel * np.sqrt(12)

    return {
        'nom': nom,
        'moyenne_mensuelle': moyenne_mensuelle,
        'ecart_type_mensuel': ecart_type_mensuel,
        'mediane': mediane,
        'rendement_annuel': rendement_annuel,
        'volatilite_annuelle': volatilite_annuelle
    }

stats_A = calculer_stats_portefeuille(rendements_A, "CONSERVATIVE (A)")
stats_B = calculer_stats_portefeuille(rendements_B, "AGRESSIF (B)")
```

### Résultats

#### 📊 PORTEFEUILLE A (Conservative)
- **Rendement mensuel moyen** : 0.94%
- **Écart-type mensuel** : 0.48%
- **Médiane** : 1.00%
- **Rendement annualisé** : 11.85%
- **Volatilité annualisée** : 1.65%

#### 📊 PORTEFEUILLE B (Agressif)
- **Rendement mensuel moyen** : 2.89%
- **Écart-type mensuel** : 4.45%
- **Médiane** : 4.70%
- **Rendement annualisé** : 40.79%
- **Volatilité annualisée** : 15.41%

### Comparaison avec la Correction

✅ **Conformité parfaite** : Les résultats obtenus correspondent exactement à ceux de la correction fournie dans le document.

---

## Question 1.2 - Visualisation Distributions

### Code Python

```python
import matplotlib.pyplot as plt
import seaborn as sns

sns.set_style("whitegrid")
fig, axes = plt.subplots(1, 2, figsize=(14, 5))

# Subplot 1 : Histogrammes superposés
ax1 = axes[0]
ax1.hist(rendements_A, bins=10, alpha=0.6, color='green', 
         edgecolor='black', label='Portefeuille A (Conservative)', density=True)
ax1.hist(rendements_B, bins=10, alpha=0.6, color='red', 
         edgecolor='black', label='Portefeuille B (Agressif)', density=True)

# Lignes moyennes
ax1.axvline(stats_A['moyenne_mensuelle'], color='darkgreen', 
            linestyle='--', linewidth=2, 
            label=f'Moyenne A = {stats_A["moyenne_mensuelle"]:.2f}%')
ax1.axvline(stats_B['moyenne_mensuelle'], color='darkred', 
            linestyle='--', linewidth=2, 
            label=f'Moyenne B = {stats_B["moyenne_mensuelle"]:.2f}%')

ax1.set_title('Distributions rendements mensuels', fontsize=12, fontweight='bold')
ax1.set_xlabel('Rendement mensuel (%)')
ax1.set_ylabel('Densité')
ax1.legend(fontsize=9)
ax1.grid(True, alpha=0.3)

# Subplot 2 : Boxplots comparatifs
ax2 = axes[1]
data_boxplot = [rendements_A, rendements_B]
bp = ax2.boxplot(data_boxplot, labels=['Portefeuille A', 'Portefeuille B'],
                 patch_artist=True, widths=0.6)

# Couleurs boxplots
colors = ['lightgreen', 'lightcoral']
for patch, color in zip(bp['boxes'], colors):
    patch.set_facecolor(color)

ax2.set_title('Boxplots comparatifs (outliers visibles)', 
              fontsize=12, fontweight='bold')
ax2.set_ylabel('Rendement mensuel (%)')
ax2.grid(True, alpha=0.3, axis='y')
ax2.axhline(0, color='black', linestyle=':', linewidth=1)

plt.tight_layout()
plt.savefig('distributions_portefeuilles.png', dpi=300, bbox_inches='tight')
plt.show()
```

### Observations

- Le **Portefeuille A** présente une distribution concentrée autour de 1% avec une faible dispersion
- Le **Portefeuille B** montre une distribution plus large avec des outliers négatifs significatifs
- Les boxplots révèlent que le Portefeuille B a une volatilité beaucoup plus importante

✅ **Conformité** : La visualisation suit exactement les spécifications de la correction.

---

## Question 1.3 - Value at Risk (VaR 95%)

### Formule VaR Paramétrique

La VaR paramétrique sous hypothèse de normalité est calculée comme suit :

\[ \text{VaR}_{95\%} = \mu - 1.645 \times \sigma \]

où 1.645 est le quantile à 5% de la loi normale standard.

### Code Python

```python
def calculer_var_portefeuille(stats_dict, capital, alpha=0.05):
    # Quantile normal standard pour alpha=5% (queue gauche)
    z_alpha = stats.norm.ppf(alpha)  # ≈ -1.645

    # a) VaR mensuelle (%)
    var_mensuelle_pct = stats_dict['moyenne_mensuelle'] + z_alpha * stats_dict['ecart_type_mensuel']

    # b) VaR annuelle (%)
    var_annuelle_pct = stats_dict['rendement_annuel'] + z_alpha * stats_dict['volatilite_annuelle']

    # c) VaR en perte monétaire (€)
    var_mensuelle_euros = capital * (var_mensuelle_pct / 100)
    var_annuelle_euros = capital * (var_annuelle_pct / 100)

    return {
        'var_mensuelle_pct': var_mensuelle_pct,
        'var_annuelle_pct': var_annuelle_pct,
        'var_mensuelle_euros': var_mensuelle_euros,
        'var_annuelle_euros': var_annuelle_euros
    }

capital = 500000
var_A = calculer_var_portefeuille(stats_A, capital)
var_B = calculer_var_portefeuille(stats_B, capital)

# d) Test normalité (Shapiro-Wilk)
stat_A, p_value_A = stats.shapiro(rendements_A)
stat_B, p_value_B = stats.shapiro(rendements_B)
```

### Résultats VaR

#### 💰 CAPITAL INVESTI : €500,000
#### 🚨 PERTE MAX TOLÉRÉE CLIENT : €50,000 (-10%)

#### 📉 PORTEFEUILLE A (Conservative)
- **VaR 95% mensuelle** : 0.15% → €764
- **VaR 95% annuelle** : 9.13% → €45,649
- **Contrainte respectée** : ✓ **OUI** (€45,649 < €50,000)

#### 📉 PORTEFEUILLE B (Agressif)
- **VaR 95% mensuelle** : -4.42% → €-22,118
- **VaR 95% annuelle** : 15.45% → €77,231
- **Contrainte respectée** : ✗ **NON** (€77,231 > €50,000)

### Test de Normalité (Shapiro-Wilk)

#### Portefeuille A :
- **Statistique Shapiro** : 0.9135
- **P-value** : 0.0003
- **Conclusion** : ✗ Les données s'écartent de la loi normale (p < 0.05) → VaR paramétrique moins fiable

#### Portefeuille B :
- **Statistique Shapiro** : 0.9219
- **P-value** : 0.0012
- **Conclusion** : ✗ Les données s'écartent de la loi normale (p < 0.05) → VaR paramétrique moins fiable

### Interprétation

- Seul le **Portefeuille A respecte la contrainte client** avec une VaR annuelle de €45,649
- Le **Portefeuille B dépasse largement** la perte maximale tolérée (€77,231 vs €50,000)
- Les **tests de normalité indiquent** que les rendements ne suivent pas parfaitement une loi normale, ce qui peut limiter la fiabilité de la VaR paramétrique

✅ **Conformité** : Les calculs correspondent à la correction, avec les mêmes conclusions sur la contrainte client.

---

## Question 1.4 - Ratio Sharpe et Recommandation

### Formule Ratio Sharpe

\[ \text{Sharpe} = \frac{R_{annuel} - r_f}{\sigma_{annuel}} \]

avec \( r_f = 3\% \) (taux sans risque OAT 10 ans).

### Code Python

```python
taux_sans_risque = 3.0  # % annuel

# Calcul Ratio Sharpe
sharpe_A = (stats_A['rendement_annuel'] - taux_sans_risque) / stats_A['volatilite_annuelle']
sharpe_B = (stats_B['rendement_annuel'] - taux_sans_risque) / stats_B['volatilite_annuelle']
```

### Résultats

#### 📊 RATIO SHARPE (Rendement ajusté du risque)

**Portefeuille A :**
- Sharpe = (11.85 - 3.00) / 1.65
- **Sharpe = 5.35**
- ✓ **Excellent** (Sharpe > 1)

**Portefeuille B :**
- Sharpe = (40.79 - 3.00) / 15.41
- **Sharpe = 2.45**
- ✓ **Excellent** (Sharpe > 1)

### 🎯 RECOMMANDATION CLIENT FINALE

#### Tableau Comparatif

| Critère | Portefeuille A | Portefeuille B |
|---------|----------------|----------------|
| Rendement annuel | 11.85% | 40.79% |
| Volatilité annuelle | 1.65% | 15.41% |
| VaR 95% (€) | €45,649 | €77,231 |
| Contrainte respectée | ✓ OUI | ✗ NON |
| Ratio Sharpe | 5.35 | 2.45 |
| Normalité (p-value) | 0.0003 | 0.0012 |

#### 💡 RECOMMANDATION FINALE

✅ **PORTEFEUILLE A (Conservative) RECOMMANDÉ**

**Justification :**
1. **Respect de la contrainte risque** : Seul le Portefeuille A respecte la VaR annuelle ≤ €50,000 avec €45,649 de perte potentielle maximale
2. **Meilleur ratio risque/rendement** : Sharpe de 5.35 vs 2.45, indiquant un rendement excédentaire par unité de risque bien supérieur
3. **Volatilité maîtrisée** : Avec seulement 1.65% de volatilité annualisée, le portefeuille A offre une stabilité remarquable
4. **Rendement attractif** : Malgré un profil conservateur, un rendement annualisé de 11.85% reste très compétitif
5. **Adéquation au profil client** : Pour un client avec une tolérance de perte limitée à 10%, le Portefeuille A est le seul choix viable

Le **Portefeuille B**, bien qu'offrant un rendement potentiel beaucoup plus élevé (40.79%), dépasse significativement la contrainte de risque du client et présente une volatilité 9 fois supérieure. Il ne convient donc pas au profil de risque spécifié.

**Note** : Les tests de normalité (Shapiro-Wilk) indiquent que les distributions s'écartent légèrement de la loi normale (p-values < 0.05), suggérant qu'une VaR historique ou par simulation Monte Carlo pourrait compléter cette analyse pour plus de robustesse.

---

## 📊 Tableau Récapitulatif Complet

| Métrique | Portefeuille A | Portefeuille B | Correction Attendue |
|----------|----------------|----------------|---------------------|
| Rendement mensuel moyen | 0.94% | 2.89% | ✅ Conforme |
| Écart-type mensuel | 0.48% | 4.45% | ✅ Conforme |
| Médiane | 1.00% | 4.70% | ✅ Conforme |
| Rendement annualisé | 11.85% | 40.79% | ✅ Conforme |
| Volatilité annualisée | 1.65% | 15.41% | ✅ Conforme |
| VaR 95% mensuelle | 0.15% | -4.42% | ✅ Conforme |
| VaR 95% annuelle | 9.13% | 15.45% | ✅ Conforme |
| VaR 95% annuelle (€) | €45,649 | €77,231 | ✅ Conforme |
| Ratio Sharpe | 5.35 | 2.45 | ✅ Conforme |
| Test Shapiro (p-value) | 0.0003 | 0.0012 | ✅ Conforme |
| Recommandation | ✓ OUI | ✗ NON | ✅ Conforme |

---

## ✅ Conclusion Partie 1

Tous les calculs et résultats obtenus sont **parfaitement conformes** à la correction fournie dans le document. L'analyse complète démontre que :

1. Les **statistiques descriptives** sont correctement calculées avec les formules appropriées
2. La **VaR paramétrique** est bien implémentée selon la méthodologie standard
3. Le **ratio Sharpe** permet d'évaluer efficacement le rendement ajusté du risque
4. La **recommandation** est basée sur des critères quantitatifs objectifs et conformes aux exigences du client

Le **Portefeuille A (Conservative)** s'impose comme le choix optimal pour ce client en raison de sa conformité aux contraintes de risque et de son excellent ratio Sharpe.

---
---
# PARTIE 2
---
---

## Théorème de Bayes et Scoring Crédit
### Mise à jour probabilités risque avec nouvelles informations

---

## 📋 Contexte

Vous êtes **data analyst** dans le département risques d'une banque retail. Mission : construire un **système de scoring crédit dynamique** utilisant le **théorème de Bayes** pour mettre à jour la probabilité de défaut d'un emprunteur en fonction de nouveaux événements.

### Données initiales (Prior)

- **Taux défaut base** (toute population) : P(Défaut) = 5%

#### Segmentation clients :
- **Segment Premium** (30% clients) : Taux défaut 1.5%
- **Segment Standard** (50% clients) : Taux défaut 5%
- **Segment Risque** (20% clients) : Taux défaut 15%

### Événements observables (Likelihood)

| Événement | P(Événement \| Défaut) | P(Événement \| Non-défaut) |
|-----------|------------------------|----------------------------|
| Retard paiement | 80% | 10% |
| Découvert >500€ | 65% | 15% |
| Demande crédit refusée ailleurs | 55% | 8% |

---

## Question 2.1 - Calcul Bayes Manuel

### 📋 Énoncé

Un client du **segment Standard** (prior défaut = 5%) présente un **retard de paiement** ce mois.

a) Calculez P(Défaut | Retard) avec le théorème de Bayes
b) Interprétez le résultat : de combien augmente le risque ?
c) Quelle décision métier recommandez-vous ?

### Formule du Théorème de Bayes

\[ P(A|B) = \frac{P(B|A) \cdot P(A)}{P(B|A) \cdot P(A) + P(B|\neg A) \cdot P(\neg A)} \]

### Code Python

```python
import numpy as np
from scipy import stats

# Données
prior = 0.05  # P(Défaut) - Segment Standard
likelihood_defaut = 0.80  # P(Retard|Défaut)
likelihood_non_defaut = 0.10  # P(Retard|Non-défaut)

# Étape 1 : Calcul P(Retard) via loi probabilités totales
p_retard = likelihood_defaut * prior + likelihood_non_defaut * (1 - prior)

# Étape 2 : Théorème de Bayes
posterior = (likelihood_defaut * prior) / p_retard

# Facteur de multiplication du risque
facteur_multiplication = posterior / prior
```

### 🧮 CALCUL DÉTAILLÉ

#### Étape 1 : Calcul P(Retard) via loi probabilités totales

```
P(Retard) = P(Retard|Défaut) × P(Défaut) + P(Retard|Non-défaut) × P(Non-défaut)
P(Retard) = 0.80 × 0.05 + 0.10 × 0.95
P(Retard) = 0.0400 + 0.0950
P(Retard) = 0.1350 = 13.50%
```

#### Étape 2 : Théorème de Bayes

```
P(Défaut|Retard) = P(Retard|Défaut) × P(Défaut) / P(Retard)
P(Défaut|Retard) = 0.80 × 0.05 / 0.1350
P(Défaut|Retard) = 0.0400 / 0.1350
P(Défaut|Retard) = 0.2963 = 29.63%
```

### 📊 INTERPRÉTATION

- **Prior (avant retard)** : 5.0%
- **Posterior (après retard)** : 29.6%
- **Augmentation risque** : +24.6 points de pourcentage
- **Facteur multiplication** : ×5.93

⚠️ **Le retard de paiement MULTIPLIE le risque de défaut par près de 6 !**

### 💡 DÉCISION MÉTIER RECOMMANDÉE

**✓ DÉCISION** : **SURVEILLANCE RENFORCÉE**

**✓ ACTION** : 
- Monitoring hebdomadaire (au lieu de mensuel)
- Limite découvert réduite de -30%
- Révision trimestrielle du dossier

**✓ JUSTIFICATION** : 
La probabilité posterior de 29.6% franchit le seuil d'alerte de 15%, plaçant le client dans une zone de risque élevé. Une surveillance accrue est nécessaire sans pour autant bloquer immédiatement l'accès au crédit.

### Comparaison avec la Correction

✅ **Conformité parfaite** : 
- P(Défaut|Retard) = 29.63% ✓
- Facteur multiplication = ×5.93 ✓
- Décision : Surveillance renforcée ✓

---

## Question 2.2 - Mise à jour Séquentielle

### 📋 Énoncé

Le même client présente **2 semaines après** un **découvert >500€**.

a) Utilisez la probabilité posterior Q2.1 comme nouveau prior
b) Calculez P(Défaut | Retard ET Découvert)
c) Tracez un graphique d'évolution de la probabilité de défaut

### Code Python

```python
# a) Nouveau prior = posterior Q2.1
prior_2 = 0.2963  # Résultat Q2.1

# Données événement 2 (Découvert)
likelihood_defaut_2 = 0.65  # P(Découvert|Défaut)
likelihood_non_defaut_2 = 0.15  # P(Découvert|Non-défaut)

# b) Calcul Bayes pour événement 2
p_decouvert = likelihood_defaut_2 * prior_2 + likelihood_non_defaut_2 * (1 - prior_2)
posterior_2 = (likelihood_defaut_2 * prior_2) / p_decouvert
```

### 🧮 CALCUL DÉTAILLÉ

**Contexte** : Utilisation posterior Q2.1 comme nouveau prior

```
Nouveau prior P(Défaut) = 0.2963 (= posterior Q2.1)
P(Découvert|Défaut) = 65%
P(Découvert|Non-défaut) = 15%

P(Découvert) = 0.65 × 0.2963 + 0.15 × 0.7037
P(Découvert) = 0.2981

P(Défaut|Retard ET Découvert) = 0.65 × 0.2963 / 0.2981
P(Défaut|Retard ET Découvert) = 0.6460 = 64.60%
```

### 📊 ÉVOLUTION PROBABILITÉ DÉFAUT

| Étape | Événement | Probabilité Défaut | Augmentation |
|-------|-----------|-------------------|--------------|
| 0 | Prior initial (Segment Standard) | 5.0% | - |
| 1 | Après Retard paiement | 29.6% | +24.6 pts |
| 2 | Après Découvert >500€ | 64.6% | +35.0 pts |

**→ TOTAL : ×12.92 augmentation du risque depuis le prior initial**

### Graphique d'évolution (c)

```python
import matplotlib.pyplot as plt

# Données pour le graphique
etapes = ['0\nPrior initial\n(Segment Standard)', 
          '1\nAprès\nRetard paiement', 
          '2\nAprès\nDécouvert >500€']
probas = [5.0, 29.6, 64.6]

# Création du graphique
fig, ax = plt.subplots(figsize=(10, 6))
ax.plot(range(3), probas, marker='o', markersize=12, linewidth=3, 
        color='darkred', label='Probabilité défaut')

# Points annotés
for i, (etape, proba) in enumerate(zip(etapes, probas)):
    ax.annotate(f'{proba:.1f}%', xy=(i, proba), xytext=(0, 10),
                textcoords='offset points', ha='center', fontsize=11, 
                fontweight='bold', 
                bbox=dict(boxstyle='round,pad=0.5', facecolor='yellow', alpha=0.7))

# Seuils décision
ax.axhline(15, color='orange', linestyle='--', linewidth=2, alpha=0.7,
           label='Seuil surveillance renforcée (15%)')
ax.axhline(30, color='red', linestyle='--', linewidth=2, alpha=0.7,
           label='Seuil restriction crédit (30%)')

ax.set_xticks(range(3))
ax.set_xticklabels(etapes)
ax.set_ylabel('Probabilité défaut (%)', fontsize=12, fontweight='bold')
ax.set_title('Mise à jour séquentielle risque crédit (Théorème Bayes)',
             fontsize=13, fontweight='bold')
ax.legend(fontsize=10)
ax.grid(True, alpha=0.3)

plt.tight_layout()
plt.savefig('evolution_risque_bayes.png', dpi=300, bbox_inches='tight')
plt.show()
```

### Comparaison avec la Correction

✅ **Conformité parfaite** : 
- P(Défaut|Retard ET Découvert) = 64.60% ✓
- Évolution correcte : 5.0% → 29.6% → 64.6% ✓
- Facteur total ×12.92 ✓

---

## Question 2.3 - Fonction Générique Bayes

### 📋 Énoncé

Créez une fonction Python `bayes_update(prior, likelihood_pos, likelihood_neg)` qui :
- Calcule la probabilité a posteriori
- Inclut une docstring complète
- Gère les validations d'entrée

Testez avec les 3 événements sur un client Segment Risque (prior 15%).

### Code Python

```python
def bayes_update(prior, likelihood_pos, likelihood_neg):
    """
    Calcule probabilité a posteriori via théorème de Bayes

    Formule : P(A|B) = P(B|A) × P(A) / P(B)
    Avec P(B) = P(B|A)×P(A) + P(B|¬A)×P(¬A)

    Parameters:
        prior : float
            Probabilité a priori P(A) ∈ [0, 1]
            Exemple : 0.05 pour taux défaut 5%

        likelihood_pos : float
            Vraisemblance P(Evidence|Positive) ∈ [0, 1]
            Exemple : 0.80 pour P(Retard|Défaut)

        likelihood_neg : float
            Vraisemblance P(Evidence|Negative) ∈ [0, 1]
            Exemple : 0.10 pour P(Retard|Non-défaut)

    Returns:
        posterior : float
            Probabilité a posteriori P(A|B) ∈ [0, 1]

    Raises:
        ValueError : Si paramètres hors [0, 1]

    Examples:
        >>> # Client défaut 5%, observe retard (80% si défaut, 10% si sain)
        >>> posterior = bayes_update(prior=0.05, likelihood_pos=0.80, likelihood_neg=0.10)
        >>> print(f"P(Défaut|Retard) = {posterior:.2%}")
        P(Défaut|Retard) = 29.63%

        >>> # Mise à jour séquentielle : posterior devient nouveau prior
        >>> posterior_2 = bayes_update(prior=posterior, likelihood_pos=0.65, likelihood_neg=0.15)
        >>> print(f"P(Défaut|Retard ET Découvert) = {posterior_2:.2%}")
        P(Défaut|Retard ET Découvert) = 64.60%
    """
    # Validation inputs
    if not (0 <= prior <= 1):
        raise ValueError(f"prior doit être dans [0,1], reçu {prior}")
    if not (0 <= likelihood_pos <= 1):
        raise ValueError(f"likelihood_pos doit être dans [0,1], reçu {likelihood_pos}")
    if not (0 <= likelihood_neg <= 1):
        raise ValueError(f"likelihood_neg doit être dans [0,1], reçu {likelihood_neg}")

    # Calcul P(Evidence) via loi probabilités totales
    p_evidence = likelihood_pos * prior + likelihood_neg * (1 - prior)

    # Protection division par zéro
    if p_evidence == 0:
        return 0.0

    # Théorème de Bayes
    posterior = (likelihood_pos * prior) / p_evidence
    return posterior
```

### 🧪 TEST FONCTION - Client Segment RISQUE (prior défaut 15%)

```python
# Événements
evenements = {
    'Retard paiement': {'P(E|Defaut)': 0.80, 'P(E|Non-defaut)': 0.10},
    'Decouvert >500€': {'P(E|Defaut)': 0.65, 'P(E|Non-defaut)': 0.15},
    'Refus credit ailleurs': {'P(E|Defaut)': 0.55, 'P(E|Non-defaut)': 0.08}
}

prior_risque = 0.15

# Événement 1 : Retard
post_1 = bayes_update(
    prior=prior_risque,
    likelihood_pos=0.80,
    likelihood_neg=0.10
)

# Événement 2 : Découvert
post_2 = bayes_update(
    prior=post_1,
    likelihood_pos=0.65,
    likelihood_neg=0.15
)

# Événement 3 : Refus crédit
post_3 = bayes_update(
    prior=post_2,
    likelihood_pos=0.55,
    likelihood_neg=0.08
)
```

### 📊 RÉSULTATS TEST (Client Segment Risque)

| Étape | Probabilité Défaut |
|-------|-------------------|
| Prior initial | 15.00% |
| Après Retard | 58.54% |
| Après Découvert | 85.95% |
| Après Refus crédit | **97.68%** |

### 💡 INTERPRÉTATION

- **Risque initial** : 15% (segment risque)
- **Après 3 événements négatifs** : 97.68%
- **Multiplication risque** : ×6.5

→ **Client très haut risque, recommandation : REJET crédit ou garanties renforcées**

Avec une probabilité de défaut proche de 98%, ce client présente un risque extrême. Même avec des garanties substantielles, l'octroi de crédit serait hautement risqué.

### Comparaison avec la Correction

✅ **Conformité parfaite** : 
- Fonction avec docstring complète ✓
- Validation des paramètres ✓
- Résultats tests identiques : 15% → 58.54% → 85.95% → 97.68% ✓

---

## Question 2.4 - Matrice Confusion et Lien Bayes

### 📋 Énoncé

Sur 10,000 clients testés avec modèle retard-paiement :
- 500 défauts réels (5%)
- 400 vrais positifs détectés (TP)
- 950 faux positifs (FP)

a) Calculez Precision = TP/(TP+FP)
b) Vérifiez cohérence avec P(Défaut|Retard) Question 2.1
c) Expliquez pourquoi Precision bayésienne = Precision matrice confusion

### Code Python

```python
# Données matrice confusion
n_total = 10000
n_defauts_reels = 500  # 5% taux défaut
n_non_defauts_reels = n_total - n_defauts_reels

tp = 400  # Vrais positifs
fp = 950  # Faux positifs
fn = n_defauts_reels - tp  # 100 Faux négatifs
tn = n_non_defauts_reels - fp  # 8550 Vrais négatifs

# a) Calcul Precision
precision = tp / (tp + fp)
```

### 📊 MATRICE CONFUSION

```
                    RÉALITÉ
              Non-défaut  Défaut
PRÉD  Retard       950      400    (1350 prédictions positives)
      Pas         8550      100    (8650 prédictions négatives)
              ─────────────────
                  9500      500
```

### 🧮 CALCUL PRECISION

```
Precision = TP / (TP + FP)
Precision = 400 / (400 + 950)
Precision = 400 / 1350
Precision = 0.2963 = 29.63%
```

### 🔗 COMPARAISON AVEC BAYES Q2.1

| Méthode | Valeur | Résultat |
|---------|--------|----------|
| P(Défaut\|Retard) calculé Bayes | 0.2963 | **29.63%** |
| Precision matrice confusion | 0.2963 | **29.63%** |
| Différence | 0.0000 | **0.00 pts** |

✓ **COHÉRENCE PARFAITE** (< 1 pt différence)

### 💡 EXPLICATION LIEN BAYES ↔ PRECISION

#### Équivalence Mathématique

**THÉORÈME DE BAYES :**
```
P(Défaut|Retard) = "Parmi clients avec RETARD, quelle % sont DÉFAUTS réels ?"
```

**PRECISION (Matrice confusion) :**
```
Precision = TP / (TP + FP) = "Parmi prédictions POSITIVES, quelle % correctes ?"
```

#### Pourquoi sont-ils identiques ?

1. **Même question posée** :
   - Bayes : P(Classe vraie | Prédiction positive)
   - Precision : Proportion de vrais positifs parmi les prédictions positives

2. **Même calcul** :
   - Bayes : \( \frac{P(Retard|Défaut) \times P(Défaut)}{P(Retard)} \)
   - Precision : \( \frac{TP}{TP + FP} = \frac{400}{1350} \)

3. **En ML** :
   - Optimiser Precision = maximiser probabilités a posteriori bayésiennes
   - Naive Bayes Classifier utilise explicitement P(Classe|Features) via Bayes
   - La Precision mesure la fiabilité du classifier selon une approche fréquentiste
   - Le calcul bayésien donne la même valeur via une approche probabiliste

#### Application Pratique

Cette équivalence montre que :
- Un modèle avec haute **Precision** attribue de fortes **probabilités bayésiennes** aux vrais cas positifs
- Les métriques ML classiques sont des manifestations empiriques des principes bayésiens
- On peut interpréter la Precision comme une probabilité a posteriori empirique

### Comparaison avec la Correction

✅ **Conformité parfaite** : 
- Precision = 29.63% ✓
- Cohérence Bayes-Precision parfaite ✓
- Explication théorique complète ✓

---

## 📊 Tableau Récapitulatif Complet - Partie 2

| Question | Prior | Posterior | Facteur Multiplication | Décision |
|----------|-------|-----------|----------------------|----------|
| 2.1 | 5.00% | 29.63% | ×5.93 | Surveillance renforcée |
| 2.2 (Étape 1) | 29.63% | 64.60% | ×2.18 | Restriction crédit |
| 2.3 (Test Risque) | 15.00% | 97.68% | ×6.51 | Rejet crédit |
| 2.4 (Precision) | - | 29.63% | - | Cohérence parfaite |

---

## ✅ Conclusion Partie 2

Tous les calculs et résultats obtenus sont **parfaitement conformes** à la correction fournie dans le document. L'analyse complète démontre que :

1. Le **théorème de Bayes** permet de mettre à jour dynamiquement les probabilités de risque en fonction de nouvelles informations
2. La **mise à jour séquentielle** amplifie considérablement le risque : de 5% à 64.6% après deux événements
3. La **fonction générique** permet d'automatiser le scoring crédit bayésien avec validation robuste
4. L'**équivalence Bayes-Precision** montre le lien profond entre approches probabilistes et métriques ML

### Points Clés Métier

- Un **simple retard de paiement** multiplie le risque par **×5.93**
- L'accumulation d'événements négatifs peut faire passer un client de **5% à 97.68%** de risque
- La **Precision** d'un modèle ML est exactement la **probabilité bayésienne a posteriori**
- Le scoring dynamique permet des **décisions adaptatives** (surveillance → restriction → rejet)

---
---
# PARTIE 3
---
---

## K-Nearest Neighbors et Évaluation Modèle
### Classification crédit et optimisation hyperparamètres

---

## 📋 Contexte

Vous êtes **data scientist** dans une fintech. Mission : construire un **modèle KNN de scoring crédit** pour automatiser décisions d'octroi prêts personnels (€5K-€50K).

**Objectifs Business :**
- Maximiser **Recall** (détecter 80%+ défauts)
- Maintenir **Precision** > 60%
- Optimiser **K** via validation croisée
- Calculer **ROI** selon les coûts métier

**Coûts Métier :**
- **Perte si défaut non détecté (FN)** : €15,000 (perte moyenne principal + intérêts)
- **Coût analyse approfondie (FP)** : €500 (vérification manuelle dossier)
- **Gain si défaut détecté (TP)** : €15,000 (perte évitée)
- **Coût opportunité refus bon client (FP)** : €1,200 (marge perdue prêt non accordé)
- **Coût total par FP** : €500 + €1,200 = €1,700

---

## Question 3.1 - Génération et Exploration Dataset

### Code Génération Dataset

```python
import numpy as np
import pandas as pd
import seaborn as sns
import matplotlib.pyplot as plt

np.random.seed(42)
n_samples = 2000

# Génération des features
age = np.random.randint(25, 66, n_samples)
salaire = np.random.normal(50000, 20000, n_samples).clip(20000, 120000)
anciennete_emploi = np.random.exponential(5, n_samples).clip(0, 30)
dette_totale = np.random.normal(25000, 15000, n_samples).clip(0, 80000)
ratio_dette_revenu = dette_totale / salaire
nb_credits_actifs = np.random.poisson(1.5, n_samples).clip(0, 5)
historique_retards = np.random.poisson(2, n_samples).clip(0, 10)
score_credit = np.random.normal(650, 100, n_samples).clip(300, 850)

# Target : Défaut si combo risque élevé
defaut_proba = (
    0.05 +  # Baseline 5%
    0.15 * (ratio_dette_revenu > 0.5) +
    0.10 * (historique_retards > 3) +
    0.08 * (score_credit < 600) +
    0.05 * (nb_credits_actifs > 2)
).clip(0, 0.85)

defaut = (np.random.rand(n_samples) < defaut_proba).astype(int)

df = pd.DataFrame({
    'age': age,
    'salaire': salaire,
    'anciennete_emploi': anciennete_emploi,
    'dette_totale': dette_totale,
    'ratio_dette_revenu': ratio_dette_revenu,
    'nb_credits_actifs': nb_credits_actifs,
    'historique_retards': historique_retards,
    'score_credit_bureau': score_credit,
    'defaut': defaut
})

df.to_csv('credit_data.csv', index=False)
```

### 📊 Statistiques Descriptives

**Dataset généré :**
- **Nombre de clients** : 2,000
- **Taux de défaut** : 16.7% (334 défauts / 1,666 non-défauts)
- **Imbalance** : 83.3% / 16.7% (déséquilibre de classes)

**Distribution Classes :**
```
Classe 0 (Non-défaut) : 1,666 clients
Classe 1 (Défaut)     :   334 clients
```

### Corrélations avec Target

| Feature | Corrélation |
|---------|-------------|
| ratio_dette_revenu | +0.124 |
| dette_totale | +0.112 |
| historique_retards | +0.069 |
| anciennete_emploi | +0.043 |
| nb_credits_actifs | +0.039 |
| age | +0.009 |
| score_credit_bureau | -0.054 |
| salaire | -0.091 |

**Observations :**
- Les corrélations sont **faibles** (max 0.12), expliquant les difficultés du modèle
- `ratio_dette_revenu` et `dette_totale` sont les features les plus prédictives
- `score_credit_bureau` et `salaire` ont des corrélations négatives (attendu)

### Visualisations

```python
# Heatmap corrélation
plt.figure(figsize=(10, 8))
sns.heatmap(df.corr(), annot=True, fmt='.2f', cmap='coolwarm', center=0)
plt.title('Matrice de Corrélation - Credit Dataset')
plt.tight_layout()
plt.savefig('heatmap_correlation.png', dpi=300)

# Boxplots features principales
fig, axes = plt.subplots(1, 2, figsize=(14, 5))
sns.boxplot(x='defaut', y='ratio_dette_revenu', data=df, ax=axes[0])
axes[0].set_title('Ratio Dette/Revenu par Classe')
sns.boxplot(x='defaut', y='historique_retards', data=df, ax=axes[1])
axes[1].set_title('Historique Retards par Classe')
plt.tight_layout()
plt.savefig('boxplots_features.png', dpi=300)
```

---

## Question 3.2 - Preprocessing et Split Train/Test

### Code Python

```python
from sklearn.model_selection import train_test_split
from sklearn.preprocessing import StandardScaler

# a) Séparation features (X) et target (y)
X = df.drop('defaut', axis=1)
y = df['defaut']

# b) Split 70/30 stratifié (préserve proportion classes)
X_train, X_test, y_train, y_test = train_test_split(
    X, y, test_size=0.3, stratify=y, random_state=42
)

# c) Normalisation avec StandardScaler
scaler = StandardScaler()
X_train_scaled = scaler.fit_transform(X_train)
X_test_scaled = scaler.transform(X_test)

print(f"Shape Train : {X_train.shape}")
print(f"Shape Test : {X_test.shape}")
print(f"Distribution Train : {y_train.value_counts()}")
print(f"Distribution Test : {y_test.value_counts()}")
```

### Résultats

**Dimensions :**
- **Train set** : (1,400, 8) features
- **Test set** : (600, 8) features

**Distribution Classes :**
| Set | Non-Défaut (0) | Défaut (1) | Taux Défaut |
|-----|----------------|------------|-------------|
| Train | 1,166 | 234 | 16.7% |
| Test | 500 | 100 | 16.7% |

✅ **Stratification réussie** : Proportions identiques dans train et test.

---

## Question 3.3 - Recherche Hyperparamètre K Optimal

### Méthodologie

- **Valeurs K testées** : [1, 3, 5, 7, 9, 11, 15, 20, 25, 30]
- **Validation croisée** : 5-fold stratifié
- **Métriques** : AUC (principale), Recall, Precision

### Code Python

```python
from sklearn.model_selection import cross_val_score, StratifiedKFold
from sklearn.neighbors import KNeighborsClassifier

k_values = [1, 3, 5, 7, 9, 11, 15, 20, 25, 30]
results = []

for k in k_values:
    knn = KNeighborsClassifier(n_neighbors=k)
    cv = StratifiedKFold(n_splits=5, shuffle=True, random_state=42)

    auc_scores = cross_val_score(knn, X_train_scaled, y_train, cv=cv, scoring='roc_auc')
    recall_scores = cross_val_score(knn, X_train_scaled, y_train, cv=cv, scoring='recall')
    precision_scores = cross_val_score(knn, X_train_scaled, y_train, cv=cv, scoring='precision')

    results.append({
        'K': k,
        'AUC_mean': auc_scores.mean(),
        'AUC_std': auc_scores.std(),
        'Recall_mean': recall_scores.mean(),
        'Precision_mean': precision_scores.mean()
    })

df_results = pd.DataFrame(results)
```

### 📊 Résultats Validation Croisée

| K | AUC Mean | AUC Std | Recall Mean | Precision Mean |
|---|----------|---------|-------------|----------------|
| 1 | 0.523 | 0.023 | 0.196 | 0.212 |
| 3 | 0.526 | 0.026 | 0.077 | 0.170 |
| 5 | 0.522 | 0.031 | 0.030 | 0.113 |
| 7 | 0.530 | 0.029 | 0.017 | 0.107 |
| 9 | 0.537 | 0.018 | 0.009 | 0.092 |
| 11 | 0.551 | 0.026 | 0.009 | 0.100 |
| 15 | 0.572 | 0.022 | 0.009 | 0.117 |
| 20 | 0.587 | 0.020 | 0.000 | 0.000 |
| **25** | **0.612** | **0.020** | **0.000** | **0.000** |
| 30 | 0.610 | 0.022 | 0.000 | 0.000 |

### 🎯 K Optimal Identifié : **K = 25**

- **AUC Maximum** : 0.612 ± 0.020
- **Observation** : Recall = 0 avec seuil par défaut (problème d'imbalance)

### Graphique AUC vs K

```python
plt.figure(figsize=(10, 6))
plt.errorbar(df_results['K'], df_results['AUC_mean'], 
             yerr=df_results['AUC_std'], marker='o', capsize=5, linewidth=2)
plt.axvline(x=25, color='red', linestyle='--', label=f'K Optimal = 25')
plt.xlabel('Nombre de voisins (K)', fontweight='bold')
plt.ylabel('AUC Moyen (5-fold CV)', fontweight='bold')
plt.title('Optimisation Hyperparamètre K - Modèle KNN', fontweight='bold')
plt.grid(True, alpha=0.3)
plt.legend()
plt.tight_layout()
plt.savefig('auc_vs_k.png', dpi=300)
```

---

## Question 3.4 - Entraînement Modèle Final et Évaluation

### Code Python

```python
from sklearn.metrics import confusion_matrix, classification_report, roc_auc_score

# Entraînement avec K optimal
knn_final = KNeighborsClassifier(n_neighbors=25)
knn_final.fit(X_train_scaled, y_train)

# Prédictions test
y_pred = knn_final.predict(X_test_scaled)
y_pred_proba = knn_final.predict_proba(X_test_scaled)[:, 1]

# Matrice de confusion
cm = confusion_matrix(y_test, y_pred)
tn, fp, fn, tp = cm.ravel()

print(f"TN={tn}, FP={fp}, FN={fn}, TP={tp}")
print(classification_report(y_test, y_pred))
print(f"AUC Test : {roc_auc_score(y_test, y_pred_proba):.4f}")
```

### 📊 Matrice de Confusion (Seuil 0.5)

|  | **Prédiction 0** | **Prédiction 1** | Total |
|---|------------------|------------------|-------|
| **Réalité 0** | **498** (TN) | 2 (FP) | 500 |
| **Réalité 1** | **100** (FN) | **0** (TP) | 100 |
| **Total** | 598 | 2 | 600 |

### Métriques de Performance (Seuil 0.5)

| Métrique | Valeur | Interprétation |
|----------|--------|----------------|
| Accuracy | 83.0% | Trompeur (taux classe majoritaire) |
| Precision | 0.0% | Aucune prédiction positive correcte |
| Recall | 0.0% | Aucun défaut détecté |
| F1-Score | 0.0% | Modèle inutile |
| Specificity | 99.6% | Excellente détection non-défauts |
| **AUC Test** | **0.589** | Capacité discriminative faible |

### Classification Report

```
              precision    recall  f1-score   support

           0       0.83      1.00      0.91       500
           1       0.00      0.00      0.00       100

    accuracy                           0.83       600
   macro avg       0.42      0.50      0.45       600
weighted avg       0.69      0.83      0.76       600
```

### 💡 Analyse

Le modèle avec K=25 et seuil par défaut (0.5) est **totalement inadapté** :
- Il prédit "Non-Défaut" pour quasiment tous les cas
- **Recall = 0%** : Aucun des 100 défauts n'est détecté
- L'objectif business (Recall > 80%) est **impossible** avec ce seuil

**Cause** : Déséquilibre de classes (83%/17%) + KNN vote majoritaire avec K élevé

---

## Question 3.5 - Courbe ROC et Analyse Seuil

### Code Python

```python
from sklearn.metrics import roc_curve
import numpy as np

# Courbe ROC
fpr, tpr, thresholds = roc_curve(y_test, y_pred_proba)

# Indice Youden
youden_j = tpr - fpr
best_idx = np.argmax(youden_j)
best_threshold = thresholds[best_idx]

print(f"Seuil Optimal Youden : {best_threshold:.4f}")
print(f"TPR={tpr[best_idx]:.3f}, FPR={fpr[best_idx]:.3f}")

# Graphique ROC
plt.figure(figsize=(10, 8))
plt.plot(fpr, tpr, linewidth=2, label=f'KNN (AUC = {roc_auc_score(y_test, y_pred_proba):.3f})')
plt.plot([0, 1], [0, 1], 'k--', linewidth=1, label='Classifieur Aléatoire (AUC = 0.5)')
plt.scatter(fpr[best_idx], tpr[best_idx], s=200, c='red', marker='*', 
            label=f'Point Optimal Youden (seuil={best_threshold:.2f})', zorder=5)
plt.xlabel('False Positive Rate (FPR)', fontweight='bold')
plt.ylabel('True Positive Rate (TPR)', fontweight='bold')
plt.title('Courbe ROC - Modèle KNN Credit Scoring', fontweight='bold')
plt.legend()
plt.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig('roc_curve.png', dpi=300)
```

### 📊 Seuil Optimal Youden

- **Seuil** : 0.16
- **TPR** : ~30%
- **FPR** : ~10%
- **Interprétation** : Maximise (Sensibilité + Spécificité - 1)

### Test de Différents Seuils

| Seuil | Precision | Recall | F1-Score | TP | FP | FN | TN |
|-------|-----------|--------|----------|----|----|----|----|
| **0.08** | **18.1%** | **90.0%** | **30.1%** | **90** | **407** | **10** | **93** |
| 0.16 | 25.0% | 30.0% | 27.0% | 30 | 90 | 70 | 410 |
| 0.30 | 28.3% | 13.0% | 17.8% | 13 | 33 | 87 | 467 |
| 0.50 | 0.0% | 0.0% | 0.0% | 0 | 2 | 100 | 498 |
| 0.70 | 0.0% | 0.0% | 0.0% | 0 | 0 | 100 | 500 |

### 🎯 Recommandation Seuil Métier

Pour atteindre l'objectif business **Recall ≥ 80%**, le seuil optimal est **0.08**.

**Compromis :**
- ✅ **Recall = 90%** : Détection de 90/100 défauts (objectif dépassé)
- ⚠️ **Precision = 18.1%** : Beaucoup de faux positifs (objectif 60% non atteint)
- **F1 = 30.1%** : Équilibre faible mais acceptable vu l'objectif Recall

---

## Question 3.6 - Calcul ROI et Recommandation Business

### Formule ROI

\[ ROI = (TP \times 15,000) - (FP \times 1,700) - (FN \times 15,000) \]

Où :
- **Gain TP** = Perte évitée (€15,000 par défaut détecté)
- **Coût FP** = Analyse manuelle (€500) + Opportunité perdue (€1,200) = €1,700
- **Coût FN** = Perte complète (€15,000 par défaut non détecté)

### Comparaison des Stratégies

#### Stratégie 1 : Baseline (Accepter tout le monde)
```
TP = 0, FP = 0, FN = 100, TN = 500
ROI = 0 - 0 - (100 × 15,000) = -€1,500,000
```

#### Stratégie 2 : Modèle Seuil 0.5 (Standard)
```
TP = 0, FP = 2, FN = 100, TN = 498
ROI = 0 - (2 × 1,700) - (100 × 15,000) = -€1,503,400
```
⚠️ **Pire que le baseline** à cause des FP !

#### Stratégie 3 : Modèle Optimisé (Seuil 0.08) ✅
```
TP = 90, FP = 407, FN = 10, TN = 93

Gains TP : 90 × €15,000 = €1,350,000
Coûts FP : 407 × €1,700 = €691,900
Coûts FN : 10 × €15,000 = €150,000

ROI = €1,350,000 - €691,900 - €150,000 = +€508,100
```

### 📊 Tableau Récapitulatif ROI

| Stratégie | TP | FP | FN | ROI | Gain vs Baseline |
|-----------|----|----|----|----|------------------|
| Baseline (Tout accepter) | 0 | 0 | 100 | -€1,500,000 | - |
| Modèle seuil 0.5 | 0 | 2 | 100 | -€1,503,400 | -€3,400 |
| **Modèle seuil 0.08** | **90** | **407** | **10** | **+€508,100** | **+€2,008,100** |

### 💡 Recommandation Business Finale

✅ **ADOPTER le modèle KNN avec seuil de décision 0.08**

**Justification Stratégique :**

1. **Profitabilité Exceptionnelle** : 
   - Seule stratégie générant un profit positif (+€508k)
   - Gain de +€2M par rapport au baseline

2. **Sécurité Maximale** :
   - Recall de 90% : 9 défauts sur 10 détectés
   - Seulement 10 défauts passent inaperçus (vs 100 en baseline)

3. **Arbitrage Coût-Bénéfice Favorable** :
   - Coût FP (€1,700) << Coût FN (€15,000)
   - Ratio 1:8.8 justifie l'acceptation de nombreux faux positifs
   - Vérifier 407 dossiers manuellement = €691k << €1.5M de pertes évitées

4. **Opérationnalisation** :
   - Mettre en place un processus de vérification rapide pour les FP
   - Monitoring continu du seuil et ajustement si besoin
   - Considérer un modèle en cascade : KNN (filtre) → Expert (validation)

**Limites Identifiées :**
- Precision de 18% reste faible (beaucoup de faux positifs)
- Charge de travail manuelle élevée (407 dossiers à vérifier)
- AUC de 0.59 indique que le dataset a des limites intrinsèques

**Recommandations d'Amélioration :**
- Collecter plus de features discriminantes
- Tester d'autres algorithmes (Random Forest, XGBoost)
- Envisager SMOTE ou oversampling pour le déséquilibre
- Combiner plusieurs modèles (ensemble learning)

---

## ✅ Conclusion Partie 3

### Résultats Clés

| Élément | Valeur Obtenue | Objectif | Statut |
|---------|----------------|----------|--------|
| K Optimal | 25 | Maximiser AUC | ✅ |
| AUC Test | 0.589 | > 0.6 | ⚠️ Limite |
| Recall (seuil 0.08) | 90% | > 80% | ✅ |
| Precision (seuil 0.08) | 18.1% | > 60% | ❌ |
| ROI Optimisé | +€508,100 | Positif | ✅ |

### Points Clés Appris

1. **L'importance du seuil de décision** : Un modèle "mauvais" (seuil 0.5) devient excellent (seuil 0.08) selon le contexte métier
2. **Trade-off Precision/Recall** : Impossible d'atteindre les deux objectifs simultanément avec ce dataset
3. **ROI comme métrique ultime** : Les métriques ML doivent servir les objectifs business, pas l'inverse
4. **Déséquilibre de classes** : KNN avec K élevé tend à voter pour la classe majoritaire

---
